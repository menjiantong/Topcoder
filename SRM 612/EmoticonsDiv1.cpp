...
int f(int s,int c,int val)
{
   // cout<<s<<" "<<c<<" "<<val<<endl;
    ...
    if(res==-1)
    {
        ...
        {
            if(val+1<30)f(s,s,val+1);
            if(s>0 && val+1<30)f(s-1,c,val+1);
            if(s+c<1000 && val+1<30)f(s+c,c,val+1);
        }
        res=1;
    }
}
